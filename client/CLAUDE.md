...
`voice.js` + `voice.css`。サーバーは `ClassRoom` の `voice:join` / `voice:leave` / `voice:msg` / `rtc:signal`。
...
- **島の外（屋外）は通話にならない**（おはなし島だけが例外）。メッシュなので25人は無理で、
  クラス全員を1つの通話にするなら SFU（LiveKit 等）が要る。`voiceRoomOf()` が唯一の判定元。
