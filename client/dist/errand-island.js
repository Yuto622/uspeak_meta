// おつかい島 — the island the errand quest is walked on.
//
// Every errand is taken at the plaza, spoken at a shop somewhere else on the island and
// delivered back to the plaza. The spots here are the same coordinates the server checks
// a child's position against, read from the same missions.json, so a place a child can
// see is always a place the server agrees they are standing in.
import * as THREE from './three.module.js';
import { loadErrandData } from './errand-data.js';

const HALF_X = 30;
const HALF_Z = 25;
export const NEAR_DISTANCE = 5;         // matches island.radius in missions.json
...
export function createErrandIsland({ scene }) {
  ...
  function build() {
    if (built || !data?.island) return;
    built = true;
    const { island } = data;
    root.position.set(island.x, 0, island.z);

    // Terrain: bedrock, beach, grass, in the same layered voxel style as Willow Island.
    B(0, -3.6, 0, 64, 5.4, 54, 0x9a9270);
    B(0, -1.8, 0, 66, 1.5, 56, 0xe0cfa4);
    B(0, -0.7, 0, 61, 1.6, 51, 0x8fa65c);
    B(0, 0.05, 0, 59, 0.3, 49, 0x9cb266);
    for (let i = 0; i < 90; i += 1) {
      const a = (i / 90) * Math.PI * 2;
      D(Math.cos(a) * 31, -1.4 - rand() * 0.6, Math.sin(a) * 26.5, 1.4 + rand() * 2.4, 1.2 + rand() * 1.6, 1.4 + rand() * 2.4, i % 2 ? 0xb3a888 : 0xc7bb96);
    }

    // The dock you arrive on, and the sign that names the place.
    ...
    if (plaza) {
      ...
      B(plaza.x - 3.4, 1.6, plaza.z - 1.6, 0.3, 3, 0.3, 0x7a6448);
      B(plaza.x + 3.4, 1.6, plaza.z - 1.6, 0.3, 3, 0.3, 0x7a6448);
    ...
    // The beacon that stands over wherever the errand says to go next.
    beacon = new THREE.Group();
    beacon.visible = false;
    root.add(beacon);
    const ringMat = new THREE.MeshBasicMaterial({ color: 0xffd98a, transparent: true, opacity: 0.85, depthTest: false });
    const ring = new THREE.Mesh(new THREE.TorusGeometry(1.9, 0.12, 8, 40), ringMat);
    ...
    // Filler greenery, kept off the paths and away from every spot.
    for (let i = 0; i < 120; i += 1) {
      const x = (rand() - 0.5) * 56;
      const z = (rand() - 0.5) * 46;
      if (island.spots.some((s) => Math.hypot(x - s.x, z - s.z) < 9)) continue;
  ...
  function setTarget(spotId) {
    targetId = spotId || '';
    if (!beacon) return;
    const spot = data?.spots.get(targetId);
    beacon.visible = !!spot;
    if (spot) beacon.position.set(spot.x, 0, spot.z);
  }

  // missions.json may still be in flight when a child flies here, so finish the job
  // when it lands rather than leaving them standing on an island that does not exist.
  const ready = loadErrandData().then((d) => {
    data = d;
    if (wanted) { build(); root.visible = true; }
    return d;
  });
  ...
  function update(t, player) {
    ...
    for (const s of spots) {
      const dist = Math.hypot(player.position.x - root.position.x - s.def.x, player.position.z - root.position.z - s.def.z);
      s.npc.position.y = Math.sin(t * 1.4 + s.def.x) * 0.05;
      if (dist < 14) s.npc.rotation.y = Math.atan2(player.position.x - root.position.x - s.def.x, player.position.z - root.position.z - s.def.z);
      s.label.visible = dist < LABEL_DISTANCE;
      s.ja.visible = dist < NEAR_DISTANCE + 2;
      s.label.position.y = 3.5 + Math.sin(t * 1.6 + s.def.z) * 0.06;
    }
    if (beacon?.visible) {
  ...
  function nearest(player) {
    ...
    for (const s of spots) {
      const dist = Math.hypot(player.position.x - root.position.x - s.def.x, player.position.z - root.position.z - s.def.z);
      if (dist <= NEAR_DISTANCE && (!best || dist < best.dist)) best = { spot: s.def, dist };
    }
    return best;
  }
  ...
  function drawMap(ctx, player) {
    ...
    ctx.fillStyle = '#9cb266';
    ctx.fillRect(12, 10, 156, 120);
    const px = (x) => 90 + (x / HALF_X) * 76;
    const py = (z) => 70 + (z / HALF_Z) * 58;
    for (const s of spots) {
      const target = s.def.id === targetId;
      ctx.fillStyle = target ? '#ffd98a' : s.def.kind === 'plaza' ? '#f0e3bd' : '#3c5a4a';
      ctx.beginPath();
      ctx.arc(px(s.def.x), py(s.def.z), target ? 6 : 4, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.fillStyle = '#fff';
    ctx.beginPath();
  ...
  return {
    ...
    get spots() { return data?.island?.spots || []; },
    get target() { return targetId; },
    ensure() { build(); return built; },
    show(on) {
      wanted = !!on;
      if (on) build();
