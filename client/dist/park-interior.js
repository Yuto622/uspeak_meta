...
export function createParkInterior(ctx){
 ...
 scene.add(player);player.position.set(0,0,6.3);player.rotation.y=Math.PI;cooldown=1;fadeTime=.45;fade.classList.add('active');document.body.classList.add('in-sanctuary');hud.hidden=false;$('.location').innerHTML='<span>✦</span><b class="en">'+(b.en||b.name)+' · Indoors</b><i class="ja">'+b.name+' の なか</i>';$('.map-panel>div b').textContent=b.name;$('#area').textContent=b.name+' · 屋内';ctx.syncBuddy(true);syncActors();toast(b.name+'へようこそ。受付まで歩いて話しかけよう。');return true;}
