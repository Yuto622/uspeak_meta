...
cd server && node test/e2e/browser-racers.mjs # AURORA KART（のりもの島から開く→中のゲームが起動→島に戻る）
```
...
- 4コース／4マシン／7人のライバル、グランプリ・タイムアタック・**英単語モード**（英検5〜3級・326語）
- 帰るときは、そのゲームのヘッダーの **「← U-Speak」**。
- **同梱した外部のゲームで、1バイトも改変していません**（`client/dist/racers/`）。
  three.js のバージョンがこの世界と違うため、iframe で隔離して動かしています。
- **オンラインのタブは外してあります。** そのゲームの通信は WebSocket をこのサーバーの
  ルートにつなぎに行き、そこは教室の Colyseus の口だからです。みんなで走るのは
  これまでどおり **U-SPEAK GRAND PRIX**（スタートラインの「レースに でる」）。
- **コイン・学習記録はこちらに入りません。** そのゲームは正解の判定をブラウザ側で
  していて（語彙データがクライアントにあります）、そこから出た数字を保護者向けの
  記録に混ぜないためです。成績はそのゲーム側に別で貯まります。

差し替えるときは `client/dist/racers/` をフォルダごと置き換えて
`node client/tests/racers-manifest.mjs --write` を実行してください
（回帰テストがハッシュを照合します）。
