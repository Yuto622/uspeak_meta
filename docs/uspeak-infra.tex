...
保護者さまがスマートフォンで開くと、いちばん上に今月のまとめが出ます。

\begin{center}
\begin{tcolorbox}[enhanced,width=0.86\textwidth,colback=uspeakDeep,colframe=uspeakDeep,
  arc=4pt,left=14pt,right=14pt,top=10pt,bottom=10pt]
{\color{uspeakCream}\bfseries\large 2026年9月のまとめ}\\[1pt]
{\color{uspeakGold}\footnotesize この1か月にやったこと}\\[7pt]
{\color{uspeakCream}
\newcommand{\mcell}[2]{\begin{minipage}[t]{0.22\textwidth}
  {\footnotesize\color{uspeakGold}#1}\\[2pt]{\LARGE\bfseries #2}\end{minipage}}
\mcell{きた日}{8 日}\hfill\mcell{といた問題}{62 問}\hfill
\mcell{正解率}{78\%}\hfill\mcell{学習した時間}{41 分}}
\end{tcolorbox}
\end{center}

\vspace{-2pt}

その下に\hl{それまでの月}が最大5か月ぶん、表で並びます（月・きた日・問題・正解率・時間）。
さらに下は、これまでどおりの\hl{累計}（レベル・総XP・正解数・コイン・5技能のバランス）です。
...
\hl{といた問題} & 英検・クイズ・釣り・バトル・おつかい・AI英会話をすべて合わせた回答数。\\
...
\hl{先月とのくらべ} & 「\textbf{先月より 2日 おおい}」「先月と おなじ ペース」のように1行で出ます。\\
\hl{5技能のチャート} & きく・はなす・よむ・かく・りかい。\textbf{やっていない技能は0}になり、その下に「つぎは◯◯を やってみよう」が出ます。\\
...
先生コンソールの \hl{「保護者レポートのリンク」} のいちばん上に
\hl{「CSV でダウンロード」} があります。押すとクラス全員ぶんが1枚の表になります。
...
\hl{英検：きびしい／ふつう／やさしい} & 正解・不正解の判定の\textbf{きびしさを3段階}で。下の表で説明します。\\
\hl{「保護者レポートのリンク」} & 一人ひとりのリンクと、クラス全員ぶんの CSV。\\
\hl{めいぼを読み直す} & 授業中に名簿へ足したお子さんを、その場で反映します。\\
...
\begin{itemize}[leftmargin=1.2em,itemsep=1pt]
  ...
  \item \hl{島の名前・お店の看板・地図}も英語になります。
