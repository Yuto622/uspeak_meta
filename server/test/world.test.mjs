...
test("a night's coins are capped, and the cap survives a rejoin", () => {
  ...
  assert.deepEqual(caps, { day: dayIndex(now), battle: 40, ghost: 12, course: 0, eiken: 0, conv: 0, voice: 0 }, 'what was spent is remembered');
  ...
  assert.deepEqual(stale, { day: dayIndex(now), battle: 0, ghost: 0, course: 0, eiken: 0, conv: 0, voice: 0 });
  assert.deepEqual(sanitizeCaps(null, now), { day: dayIndex(now), battle: 0, ghost: 0, course: 0, eiken: 0, conv: 0, voice: 0 });
